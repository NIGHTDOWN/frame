...
class YValid {
    ...
    public static function isMobile($value) {
        ...
        if (strlen($value) == 11) {
            if (preg_match('/13[0123456789]{1}\d{8}|14[0123456789]{1}\d{8}|15[0123456789]{1}\d{8}|18[0123456789]{1}\d{8}|17[0123456789]{1}\d{8}/', $value)) {
                return true;
            }
            else {
                return false;
	...
	public static function isPlugin($plugin) {
		...
		if (is_string($plugin_path) && preg_match("/^[\w\-\/]+\.php$/", $plugin_path) && file_exists(ROOT . 'source/plugin/' . $plugin_path)) {
			return true;
		} else {
			return false;
		}
    ...
    public static function isHashID($h) {
        if (preg_match("/^\d+-\d+$/", $h)) {
            return true;
        }
        else {
    ...
    public static function isIDs($s_str) {
        if (preg_match("/^[0-9,]+$/u", $s_str)){
            return true;
        }
        else {
            return false;
