...
class Upload {
    ...
    public function upload($name, $module='', $path='', $params = array()) {
        ...
        $newName = substr(md5(time().YHandle::getRndChar(12)),8,16);
    ...
    public function checkContent($uploadInfo) {
        ...
        $file_content = $this->readover(ROOT.$uploadInfo['source']);
        if (empty($file_content)) {
            @unlink(ROOT.$uploadInfo['source']);
    ...
    private function getFileSize($file) {
        if (empty($file)) {
            ...
            if (!file_exists(ROOT.$file)) {
                ...
                if (filesize(ROOT.$file)>$this->params['maxSize']){
                    return false;
                }
                else{
                    return true;
