 ...
 * Author:	Don Duvall <don@deduvall.com>
 * Copyright:	2003-2005 by Paul Lockaby
 * Copyright:	2005-2006 Mark Dickenson
 * Copyright:	2012-2018 Don Duvall
 *
...
class Template_Lite_Compiler extends Template_Lite {
	...
	var $_extends_stack     =   array();
	var $_region_stack      =   array();
	...
	function __construct() {
		...
		// [$bar]
		// [#bar#]
		$this->_var_bracket_regexp = '\[(?:\$?\w+|\#\w+\#|\\\'\w+\\\'|\"w+\")(?:\.(?:\$?\w+|\#\w+\#|\\\'\w+\\\'|\"w+\"))*\]';
...
//		$this->_dvar_regexp = '\$[a-zA-Z0-9_]{1,}(?:' . $this->_var_bracket_regexp . ')*(?:' . $this->_var_bracket_regexp . ')*';
		$this->_dvar_regexp = '\$[a-zA-Z0-9_]{1,}(?:' . $this->_var_bracket_regexp . ')*(?:(?:\.|\->)\$?\w+(?:' . $this->_var_bracket_regexp . ')*(?:' . $this->_var_bracket_regexp . ')*)*';
...
//		$this->_func_regexp = '[a-zA-Z_]\w*';
	...
	function preg_cb_return_0($matches) {
		return '';
	}
	...
	function _compile_file($file_contents) {
		...
		$file_contents = preg_replace_callback("/({$ldq}\*.*?\*{$rdq})/s", array($this, 'preg_cb_return_0'), $file_contents);
		...
		for ($i = 0, $for_max = $count_tags; $i < $for_max; $i++) {
			...
			$compiled_tags[] = $this->_compile_tag($this->_fixtag($tags[$i]));
...
*/
		...
		$rep='/(?:(' . $this->_var_regexp . '|' . $this->_svar_regexp . '|\/?' . $this->_func_regexp . ')(' . $this->_mod_regexp . '*)(?:\s*[,\.]\s*)?)(?:\s+(.*))?/xs';
		preg_match_all($rep, $tag, $_match);
		// $tag=str_replace(array('{','}'),array(),$tag);
		
		$mat2=preg_split($rep,$tag);
		...
		$_result = $this->_parse_eval_variables($_match[1],$_match[2], $mat2);
	...
	function _fixtag($tag){
		
		if(strpos($tag,'vo_list')!==false){
			return $tag;
		}
		$str=str_replace(array('{','}'),array(),$tag);
		return $str;
		preg_match_all('/[\{]{1}([\$]{1}.*?)[\}]/',$tag,$_match_value);
	
		if(is_array($_match_value[1]) && sizeof($_match_value[1])>0){
			$_match='';
		...
		$str=str_replace($_match_value[0],$_match,$tag);
...
function _parse_volist_arguments($arguments){
	
	$_match		= array();
		$_result	= array();
		$_variables	= array();
		preg_match_all('!\{\s*(.*?)\s*\}!s', $arguments, $_match);
		$v2=preg_split('!\{\s*(.*?)\s*\}!s', $arguments);
		/*
		   Parse state:
		...
		*/
		...
		foreach($_match[1] as $i=>$value) {
						...
						if(!preg_match_all('/(?:(' . $this->_var_regexp . '|' . $this->_svar_regexp . ')(' . $this->_mod_regexp . '*))(?:\s+(.*))?/xs', $value, $_variables)) {
							...
							$v3=preg_split('/(?:(' . $this->_var_regexp . '|' . $this->_svar_regexp . ')(' . $this->_mod_regexp . '*))(?:\s+(.*))?/xs', $value);
						
						
							
							$_result[$i] = $this->_parse_variables2($_variables[1], $v3);
						
						}
						
					//	$_result[$i]=str_replace(array('{',"}"),array('',''),$_result[$i]);	
			$str.=$v2[$i].'@!'.$_result[$i]."!@";
		}
	
		if(sizeof($v2)>sizeof($_match[1])){
		$str.=$v2[sizeof($v2)-1];
		}
...
function _parse_variables2($variables, $modifiers) {
		...
		foreach($variables as $key => $value) {
			...
			if (empty($modifiers[$key])) {
				...
				$_result .= "'{$modifiers[$key]}'^[".$this->_parse_variable($value)."]^" ;
				
			}
		}
		
		//$str=substr($_result, 1);
		$str=$_result;
		$str=str_replace(array('{','}'),array('',''),$str);
		$str=str_replace(array('^[',']^'),array('{','}'),$str);
	...
	function _parse_function($function, $modifiers, $arguments) {
		switch ($function) {
			...
			case 'foreach':
				...
				$_args=$this->_matchforeach($arguments);
				/*$_args = $this->_parse_arguments($arguments);
				*/
				...
				*/
				...
				isset($_args['key']) ? $_args['key'] = $_args['key']." => " : $_args['key'] = '';
				
				$_result = '<?php if (count((array)' . $_args['from'] . ')): foreach ((array)' . $_args['from'] . ' as ' . $_args['key']  . $_args['value'] . '): ?>';	
				/*$_result = '<?php if (count($this->_vars[' . $_args['from'] . '])): foreach ($this->_vars[' . $_args['from'] . '] as ' . $_args['key'] . '$this->_vars[\'' . $_args['value'] . '\']): ?>';*/
				return $_result;
				break;
			case 'foreachelse':
			...
			case '/if':
				...
				break;
			case 'assign':
			$_args = $this->_parse_assign_arguments($arguments);
			...
			case 'case':
				...
				break;
			case 'region':
			    if (!function_exists('compile_region_start')) {
					require_once(TEMPLATE_LITE_DIR . "internal/compile.compile_region_start.php");
				}
				return compile_region_start($arguments, $this);
			    break;
			case '/region':
			    if (!function_exists('compile_region_end')) {
					require_once(TEMPLATE_LITE_DIR . "internal/compile.compile_region_end.php");
				}
				return compile_region_end($arguments, $this);
			    break;
			case 'extends':
			    $_args = $this->_parse_arguments($arguments);
			    
				// Add the file name into the extends stack
				array_push($this->_extends_stack, $_args['file']);
				break;
			case '/extends':
				// Include the top of the extends stack
				if (!function_exists('compile_include')) {
					require_once(TEMPLATE_LITE_DIR . "internal/compile.include.php");
				}
				// pop the last extends off from the stack
				$file = array_pop($this->_extends_stack);
				/* print('<?php print("including \'' . $file . '\'"); ?>'); */
				return compile_include('file=' . $file . '', $this);
				break;
			case 'config_load':
				$_args = $this->_parse_arguments($arguments);
				if (empty($_args['file'])) {
...
function _matchforeach($str){
	$data=explode('as',$str);
	
	$ret['from']=$this->_get_tpl_index_name($data[0]);
	$v=explode('=>',$data['1']);
	if(sizeof($v)==1){
		$ret['value']=$this->_get_tpl_index_name($v[0]);
	}else{
		$ret['key']=$this->_get_tpl_index_name($v[0]);
		$ret['value']=$this->_get_tpl_index_name($v[1]);
	}
	return $ret;
}
	...
	function _get_tpl_index_name($arguments){
		if(!preg_match_all('/(?:(' . $this->_var_regexp . '|' . $this->_svar_regexp . ')(' . $this->_mod_regexp . '*))(?:\s+(.*))?/xs', $arguments, $_variables)) {
							return  $arguments;
						} else {
							return $this->_parse_variables($_variables[1], $_variables[2]);
						}
	}
	function _parse_assign_arguments($arguments) {
		...
		//"\"([^\"]*)\""
		preg_match_all('/var=[\'\"]{1}(.*?)[\'\"]{1}/is', $arguments,$_match);
		
		if(sizeof($_match[1])){
			$_result['var']=$_match[1][0];
		}
		preg_match_all('/value=[\'\"]{1}(.*?)[\'\"]{1}/is', $arguments,$_match);
		if(sizeof($_match[1])){
			
			if(!preg_match_all('/(?:(' . $this->_var_regexp . '|' . $this->_svar_regexp . ')(' . $this->_mod_regexp . '*))(?:\s+(.*))?/xs', $_match[1][0], $_variables)) {
							$_result['value'] = $_match[1][0];
						} else {
							$_result['value'] = $this->_parse_variables($_variables[1], $_variables[2]);
						}
			
			
			
		}else{
			preg_match_all('/vo_list[\(]{1}(.*?)[\)]{1}/is', $arguments,$_match);
	...
	function _parse_argument_to($arguments) {
		$_match		= array();
		$_result	= array();
		$_variables	= array();
	...
	function _parse_eval_variables($variables, $modifiers,$m3) {
		...
		/*	$_result .= $m3[$key].$this->_parse_variable($tag_variable);*/
			
			if (empty($modifiers[$key])) {
			
				$_result .= $m3[$key].$this->_parse_variable($tag_variable)."{$modifiers[$key]}";
			} else {
				$_result .= $m3[$key].$this->_parse_modifier($this->_parse_variable($tag_variable), $modifiers[$key]);
			}
		
		}
			if(sizeof($m3)>sizeof($variables)){
				$_result.=$m3[sizeof($m3)-1];
			}	
			//$_result=str_replace(array('{','}'),array(),$_result);
		return $_result;
	}
...
//			preg_match_all('/(?:[^\\\]' . $this->_dvar_regexp . '[^\\\])/', $variable, $_expand);
	...
	function _compile_variable($variable, $accessor = null, $previous = null) {
		$_result	= "";
...
//					$_result .= "['$var']";
	...
	function _plugin_exists($function, $type) {
		...
		if (isset($this->_plugins[$type][$function])
			...
			&& (
				is_object($this->_plugins[$type][$function][0])
				|| class_exists($this->_plugins[$type][$function][0])
			)
			&& method_exists($this->_plugins[$type][$function][0], $this->_plugins[$type][$function][1])) {
