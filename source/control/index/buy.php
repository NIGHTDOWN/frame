...
class buy extends indexbase
{
  ...
  public function control_run()
  {
    ...
    $address = T('user_address')->join_table(array('t'=>'province','provinceid','provinceID'),1)->join_table(array('t'=>'city','cityid','cityID'),1)->join_table(array('t'=>'area','areaid','areaID'),1)->order_by(array('f'=>'addtime','s'=>'down'))->set_where(array('uid'=>$uid))->get_all();
  ...
  /*  $cid   = 'cid in ('.implode(',',$get['product_id']).')';*/
    ...
    $order = T('cart')->join_table(array('t'=>'merchant','muid','muid'),1)->group_by('v.muid')->set_field('v.muid,merchantname')->set_where(array('uid'=>$uid,'old'=>0))->set_where($get,'in')->get_all();
    ...
    foreach ($order as $i=>$v) {
      $plist = T('product')->join_table(array('t'=>'cart','productid','productid'),1)->set_where(array('pflag'   =>1,'shelves' =>1,'status'  =>0,'muid'    =>$v['muid'],'cart.uid'=>$uid))->set_where($get,'in')->order_by(array('s'=>'down','f'=>'v.muid'))->get_all();
  ...
  public function control_sureforcart()
  {
    ...
    /*$cid   = implode(',',$get['cid']);*/
    $order = T('cart')->group_by('muid')->set_where(array('uid'=>$uid,'old'=>0))->set_where(array('cid'=>$get['cid']),'in')->get_all();

    foreach ($order as $i=>$v) {
      $insert = array();
      $plist = T('product')->join_table(array('t'=>'merchant','muid','muid'),1)->join_table(array('t'=>'cart','productid','productid'),1)->set_where(array('pflag'   =>1,'shelves' =>1,'status'  =>0,'muid'    =>$v['muid'],'cart.uid'=>$uid))->set_where(array('cid'=>$get['cid']),'in')->order_by(array('s'=>'down','f'=>array('muid','cart.addtime')))->get_all();
