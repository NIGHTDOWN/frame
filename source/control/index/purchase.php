   ...
   public function control_run(){
	  $get = get(array('string' => array('word'), 'int' => array('price1', 'price2')));
       ...
       /* $where2 = $where;*/
        ...
        $model = T($this->db_name)->join_table(array('t' => 'product', 'pid', 'productid'),1)->set_field('v.*,merchantname')->order_by(array('f' => 'productid', 's' => 'down'))->set_global_where($where)->join_table(array('t' => 'merchant', 'gmuid', 'muid'), 1)->join_table(array('t' => 'product_attr', 'pid', 'productid'), 1);
    ...
    public function control_detail(){
		...
		if(!$info){YOut::page404();error('产品不存在');}
		if($this->get_muid()!=$info['muid']){
			if($info['pflag']!=1){YOut::page404();error('产品未审核');}
			if($info['gcheck']!=1){YOut::page404();error('团购已失效');}
			if($info['gstime']>time()){YOut::page404();error('团购已失效');}
			if($info['getime']<time()){
