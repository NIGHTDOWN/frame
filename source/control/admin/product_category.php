...
checktop();
class product_category extends adminbase
{
    private $db_name = 'product_category';
    private $key='catid';
    private $allkey=array(
        'int'=>array('elite','flag','parentid','orders','ptype'=>1),
         'string'=>array('alias','catname'=>1,'detailtpl','indextpl','listtpl','metadesc','metakeyword','metatitle','pic'),
    ...
    public function control_add_view(){
        ...
        $var_array=(array('select'=>$where,'data'=>$this->cate()));
    ...
    public function control_show()
    {
        ...
        $var_array=array($c=>$data,'data'=>$this->cate());
        $this->view(null,$var_array);
    ...
    public function control_add()
    {
        ...
        $flag=$mod->add($insert);
        if($flag){
        	M('tree','am')->uptree('product_category_tree',$flag,$insert['parentid']);
